	...
	<script type="text/javascript">
		...
		function build_emps_table(result){
			...
			$.each(emps,function(index,element){
				...
				var genderTd=$('<td></td>').append(element.gender=='M'?"男":"女");
				...
				editBtn.attr('edit-id',element.empId);
				var delBtn=$('<button></button>').addClass('btn btn-danger btn-sm delete_btn')
							.append($('<span></span>').addClass('glyphicon glyphicon-trash'))
							.append('删除');
				//为删除按钮添加一个自定义的属性，来表示当前员工id
				delBtn.attr('del-id',element.empId);
		...
		function build_page_nav(result){
			...
			var nextePageLi=$('<li></li>').append($('<a></a>').append('&raquo;'));
			var lastPageLi=$('<li></li>').append($('<a></a>').append('尾页').attr('href','#'));
			if(result.extend.pageInfo.hasNextPage==false){
				nextePageLi.addClass('disabled');
				lastPageLi.addClass('disabled');
			}else{
				nextePageLi.click(function(){
					to_page(result.extend.pageInfo.pageNum+1);
				})
			...
			//遍历页码号
			$.each(result.extend.pageInfo.navigatepageNums,function(index,element){
				var numLi=$('<li></li>').append($('<a></a>').append(element));
			...
			//最后添加下一页和尾页按钮
			ul.append(nextePageLi).append(lastPageLi);
...
// 			alert("单击事件...");
		...
		function getEmp(id) {//emp_update_static
			$.ajax({
				url:'${APP_PATH}/emp/'+id,
				type:'get',
				success:function(result){
					...
					$('#empUpdateModal input[name=gender]').val([empData.gender]);//单选框会选中：[empData.gender]
...
// 			alert($(this).parents('tr').find('td:eq(1)').text());
		...
		//每个复选框都选中后将 全选的复选框自动选上
		$(document).on('click','.check_item',function(){
			//判断当前选择中的元素是否是5个，选中5个后flag为true
